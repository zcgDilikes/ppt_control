...
class PptDesktopApp:
    ...
    def _build_ui(self):
        ...
        _btn(btn_row, "浏览…", self._browse_open_ppt_path, primary=True).pack(side=tk.LEFT, padx=(0, 10))
        _btn(btn_row, "清除", self._clear_open_ppt_path, primary=False).pack(side=tk.LEFT)
        ...
        _btn(dl_tool, "在文件夹中显示所选", self._downloads_reveal_selected, primary=True).pack(
            side=tk.LEFT, padx=(0, 10)
        )
        _btn(dl_tool, "打开保存目录", self._open_save_dir, primary=False).pack(side=tk.LEFT)
    ...
    def _build_gesture_tab(self, parent, wrap: int):
        ...
        _btn(row0, "启动手势", self._gesture_start, primary=True).pack(side=tk.LEFT, padx=(0, 8))
        _btn(row0, "停止", self._gesture_stop, primary=False).pack(side=tk.LEFT, padx=(0, 8))
        tk.Checkbutton(
            row0,
            text="仅预览不执行指令",
        ...
        _btn(row3, "开始双人配对", self._gesture_start_pairing, primary=False).pack(side=tk.LEFT, padx=(0, 8))
        _btn(row3, "重新配对", self._gesture_reset_pairing, primary=False).pack(side=tk.LEFT)
